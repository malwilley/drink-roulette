...
convertToDrink backendDrink =
    let
        proportions =
            List.map convertToIngredientProportion backendDrink.contents

        drinkSize =
            intToDrinkSize backendDrink.size
    in
        if List.any (\p -> p == Nothing) proportions then
            Error "Error parsing drink contents"
        else
            case drinkSize of
                Just drinkSize ->
                    Success
                ...
                Nothing ->
                    Error "Error parsing drink size"
...
convertToIngredientProportion contentsBackend =
    let
        ingredient =
            backendIngredientToIngredient contentsBackend.ingredient
    in
        case ingredient of
            Just ingredient ->
                Just
